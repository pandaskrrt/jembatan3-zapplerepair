...
<div class="history-page">
    <div class="header">
        ...
        <h1>Riwayat Aktivitas: {item.name}</h1>
    </div>

    <div class="item-info">
        <span>Stok: {item.stock}</span>
        <span>Kategori: {item.category} / {item.subCategory}</span>
        {#if item.price}
            <span>Harga: Rp {item.price.amount.toLocaleString('id-ID')}</span>
        {/if}
    </div>
...
<style>
    .history-page { max-width: 900px; margin: 0 auto; padding: 24px; }
    .header { display: flex; align-items: center; gap: 16px; margin-bottom: 24px; }
    .header h1 { margin: 0; font-size: 1.5rem; }
    .back-btn { padding: 8px 16px; border: 1px solid #d1d5db; border-radius: 6px; background: white; cursor: pointer; }
    .back-btn:hover { background: #f3f4f6; }
    .item-info { display: flex; gap: 24px; padding: 16px; background: #f9fafb; border-radius: 8px; margin-bottom: 24px; font-size: 0.9rem; }
    .history-list { display: flex; flex-direction: column; gap: 12px; }
    .empty { text-align: center; padding: 48px; color: #9ca3af; }
    .history-card { border: 1px solid #e5e7eb; border-left: 4px solid; border-radius: 8px; padding: 16px; background: white; }
    ...
    .note { font-size: 0.85rem; color: #374151; margin-bottom: 4px; }
    ...
    :global(.change-field) { font-weight: 600; color: #374151; }
    :global(.change-old) { color: #ef4444; text-decoration: line-through; }
    :global(.change-new) { color: #10b981; font-weight: 600; }
    .history-footer { margin-top: 8px; font-size: 0.75rem; color: #9ca3af; }
